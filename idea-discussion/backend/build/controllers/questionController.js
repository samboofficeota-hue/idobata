...
export const getDebateAnalysis = async (req, res) => {
    ...
    try {
        ...
        console.error(`Error getting debate analysis for question ${questionId}:`, error);
        res.status(500).json({
...
export const getReportExample = async (req, res) => {
    ...
    try {
        ...
        console.error(`Error getting report example for question ${questionId}:`, error);
        res.status(500).json({
...
export const updateQuestionVisibility = async (req, res) => {
    try {
        ...
        const question = await SharpQuestion.findByIdAndUpdate(questionId, { isVisible }, { new: true });
        if (!question) {
