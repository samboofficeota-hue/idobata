...
### 9.5 政策の良し悪しを説明するときの情報構造（対象・目的・期待効果）

政策・法案の「良し悪し」を説明する際、みらい議会では次のような観点で情報が構造化されている（設計書・外部紹介記事から整理）。

| 観点 | みらい議会での扱い | 備考 |
|------|---------------------|------|
| **対象（だれ）** | **影響を受けうる層** として言及（サービス紹介等）。誰が影響を受けるかを明示する設計。 | DB の `bill_contents` は title / summary / content のみ。対象者情報は本文（Markdown）内や別 UI で扱っている可能性。 |
| **目的** | **改正が必要な理由**、**ポイント解説** に該当。法案の背景・狙いを説明する部分。 | 貧困層の支援・新産業創出・防災など、政策目的を「なぜやるか」として整理。 |
| **期待効果** | **影響・効果の説明** が難易度別コンテンツ要件で明示（9.2 の「コンテンツ構成」）。 | 各難易度で「ポイント解説」と「影響・効果の説明」を保持する設計。 |

**設計書で確認できること**

- 難易度別コンテンツ要件（`議案コンテンツ難易度切り替え機能要件定義`）：**タイトル / 要約 / 本文 / ポイント解説 / 影響・効果の説明** を管理。
- 実装計画の `bill_contents` スキーマでは、上記のうち「タイトル・要約・本文」がカラムで、**ポイント解説・影響・効果は Markdown 本文（content）内のセクション**として格納する運用が想定される。
- サービス紹介では「賛否の論点」「影響を受けうる層」「チームみらいの立場」などが議案コンテンツの要素として挙げられている。

**idobata への活用のしかた**

- 課題・解決策やシャープな問いを**説明・生成するとき**に、「対象（だれ向けか）」「目的（何のための施策か）」「期待効果（どう変わるか）」の3観点をプロンプトや出力形式に含めると、**政策の良し悪しの説明が一貫しやすく**なる。
- 対話AIの参考情報（REF）に渡す課題・解決策に、短い「対象・目的・期待効果」のメモを添えると、LLM が「誰に・何のために・どんな効果があるか」を押さえた返答をしやすくなる。
- シャープな問いの生成で「現状→望ましい状態」に加え、「どの層を主な対象とするか」「どのような効果を期待するか」を問いに織り込む設計も検討できる。

#### HMW アプローチと 対象・目的・期待効果 の共存

**結論: 共存して成立する。** 役割が違うので、両方を組み合わせられる。

| 観点 | HMW（How might we...） | 対象・目的・期待効果 |
|------|------------------------|------------------------|
| **役割** | **問いの形式・マインドセット**。開かれた問い、現状→望ましい状態、手段は示さない。 | **説明・評価のときの情報の切り口**。誰に・何のため・どんな効果かを構造化する。 |
| **レベル** | 「どう問うか」（問いの型、Design Thinking の精神）。 | 「何を押さえて説明するか」（政策の良し悪しを語るときの観点）。 |

**共存のしかた**

- **問いの文面は HMW のまま**でよい。「私たちはどうすれば…」「現状は○○、それをこうしたい」「それが○○になるのは望ましいだろうか？」という**形式**を維持する。
- **問いの内容に 対象・目的・期待効果を織り込む**＝「だれ向けの変化か」「何のための議論か」「どんな効果を想定するか」が、HMW の一文の中に自然に含まれるようにする。例：「**若年層が**（対象）、**雇用の安定**（目的）に向けて、**非正規と正規の格差**（現状）を**同一労働同一賃金で埋める**（望ましい状態／期待効果）のは望ましいだろうか？」。
- **メタ情報として持つ**：問いテキストは短く HMW に保ち、別フィールド（tagLine・tags の拡張や、将来の「対象・目的・期待効果」メモ）で 対象・目的・期待効果を補足してもよい。対話AIの REF や政策説明では、このメタ情報を参照する。

**プロンプトでの両立**

- システムプロンプトで「HMW 形式で、現状・望ましい状態を詳しく、手段は示さない」はそのままにする。
- そのうえで「各問いが、**主にどの層（対象）**を念頭にしているか、**何の目的**（貧困支援・防災・産業など）に触れているか、**どのような効果**を想定しているかを、問いの文言や tagLine に反映させる」と一文追加すれば、HMW の型を崩さずに 対象・目的・期待効果 を反映した問いを生成しやすい。

### 9.6 シャープな問い 6 件生成で使っている情報と「分類」のしかた

**使っている情報（現状）**

| 入力 | 使っているか | 備考 |
|------|----------------|------|
| **課題（Problem）の statement** | ✅ 唯一の入力 | `Problem.find({ themeId }, "statement")` で取得し、改行区切りで LLM に渡す。 |
| テーマ名・テーマ説明 | ❌ | 渡していない。 |
| 解決策（Solution） | ❌ | 渡していない。 |
| 対象・目的・期待効果 | ❌ | 渡していない。 |

→ つまり「**そのテーマに紐づく課題文の一覧だけ**」で 6 問を生成している。

**6 個の「分類」のしかた**

- **分類に使っている入力軸はない**。テーマや対象・目的・期待効果で事前に 6 枠を切っているわけではない。
- **プロンプトによる形式の区別**だけがある：
  - **1〜3 番**: 「現状はこう」「それをこうしたい」を詳しく書く。手段は示さない（HMW の典型的な形）。
  - **4〜6 番**: 「現状は○○だが、それが○○になるのは望ましいだろうか？」形式。望ましい状態そのものの妥当性・合意の有無を問う。
- **誰が何番になるか**は LLM 任せ。課題が複数ある場合、「Consolidate similar problems into broader HMW questions」に従って類似をまとめつつ、6 つの問いを立てている。
- **DB には「何番目か」「1–3 型か 4–6 型か」は保存しない**。保存するのは `questionText`, `tagLine`, `tags`, `themeId` のみ。`tags` は LLM が自由に付ける 2 語のカテゴリで、事前定義した分類体系ではない。

**まとめ**

- **情報**: 課題の statement のみで分類しているのではなく、「課題一覧から 6 問を立てる」というタスクを LLM に任せている。
- **分類**: 入力側の「対象・目的・期待効果」などで 6 枠を切っているわけではなく、**出力の形式**（1–3 は現状→望ましい状態、4–6 は〜望ましいだろうか）で 2 種類に分けているだけ。内容の振り分け（どの課題がどの問いに対応するか）は LLM の判断に委ねられている。

**※ 2025年2月 仕様更新**: 6 件を「HMW ＋ 対象・目的・期待効果のセット」で分類する形に変更。入力に解決策（Solution）を追加。出力は `questionText`（50字以内）・`tagLine`（15字以内）・`contextSets`（配列: 各要素に target / purpose / expectedEffect）。`contextSets` は対話AIの参考情報（REF）で課題・解決策とあわせて投げかけに活用。詳細は 9.8 参照。

### 9.7 テーマのジャンル（農業・福祉・医療・教育・防災など）について

**idobata の現状**

- テーマ（Theme）には **ジャンル／カテゴリのフィールドはない**。  
  - スキーマ: `title`, `description`, `slug`, `isActive`, `customPrompt`, `disableNewComment`, `clusteringResults` のみ（`idea-discussion/backend/models/Theme.js`）。
- シャープな問いの生成でも、**課題の statement 一覧だけ**を渡しており、テーマ名・説明すら使っていない。  
- したがって「農業」「福祉」「医療」「教育」「防災」といった**事前のジャンル設定は含んでいない**。問いの内容は課題文とプロンプトの形式指示に依存している。

**みらい議会でのジャンル相当の扱い**

- 議案（bills）自体に **genre カラムはない**（MVP DB スキーマ: `name`, `headline`, `description`, `status`, `body_markdown` など）。
- **議案タグ機能**（`docs/20251011_1430_議案タグ機能DB設計.md`）で、議案ごとに**タグ**を付与する設計になっている。
  - `tags` テーブル: `id`, `label`（表示名）。  
  - `bills_tags`: 議案とタグの多対多の中間テーブル。
- サービス紹介などでは、議案を「防災」「人事」「暮らし」などの**ジャンルで分類**していると説明されており、これらは **タグの label** として運用されていると考えられる（例: 防災＝気象業務法改正など、人事＝公務員給与改正など、暮らし＝犯罪防止・DV 防止など）。
- 設計書の「今後の拡張案」には、タグに `category` カラムを追加する案も記載されている。

**対比のまとめ**

| 観点 | idobata（現状） | みらい議会 |
|------|------------------|------------|
| テーマ／議案にジャンルを持つか | 持たない（Theme に genre なし） | 議案に**タグ**でジャンル相当を付与（防災・人事・暮らしなど） |
| シャープな問い生成での利用 | 課題文のみ。テーマのジャンルは使っていない | （問い生成は別機能のため直接の対応なし） |

idobata で「農業・福祉・医療・教育・防災」などのジャンルを効かせたい場合は、Theme に `genre` や `category` を追加するか、テーマに紐づく**タグ**を導入し、シャープな問い生成のプロンプトに「このテーマのジャンルは○○」と渡す形が考えられる。

### 9.8 シャープな問いの新仕様（HMW ＋ 対象・目的・期待効果）

**設計方針（2025年2月）**

1. **HMW アプローチ**（どうありたいか・未来ビジョン）を 1 問の軸とする。
2. 同じ HMW に対し、**対象・目的・期待効果**を 1 セットとみなすと複数セットありうる（対象が違う、目的が違う、空欄あり可）。これを配列データで保持する。
3. **アウトプット**（ユーザー向け）: HMW に基づく **見出し（tagLine）** と **問い（questionText）**。  
   - tagLine: 最大 15 文字。  
   - questionText: 50 文字以内。望ましい姿の提示と現実との差を端的に。
4. **2〜4 のセット（contextSets）** は、Problem / Solution とあわせて**ユーザーとの AI 対話の投げかけ情報**（REF）に使う。

**実装**

- **モデル**（`SharpQuestion.js`）: `contextSets: [{ target, purpose, expectedEffect }]` を追加。各要素は空文字可。
- **生成**（`questionGenerator.js`）: 入力に課題＋解決策を渡す。出力は 6 件、各件に `question`（50字以内に切り詰め）・`tagLine`（15字以内）・`contextSets`（1 件以上、複数可）。
- **対話**（`chatController.js`）: 問い単位・テーマ単位の参考情報に、各問いの `contextSets` を「対象・目的・期待効果（投げかけのヒント）」として追記。LLM が課題・解決策とあわせて視点を投げかけやすくする。

**PolicyDraft への代表 contextSet の反映**

- **抽出**: SharpQuestion の `contextSets` のうち、少なくとも1つ非空フィールドがある**最初のセット**を「代表」として採用（`policyGenerator.js` の `getRepresentativeContextSet`）。
- **PolicyDraft スキーマ**: 任意フィールド `representativeContextSet: { target, purpose, expectedEffect }` を追加。既存ドラフトは未設定のまま。
- **生成時**: 代表セットがある場合のみプロンプトに「対象・目的・期待効果」を渡し、PolicyDraft 保存時に `representativeContextSet` をセット。なければ渡さず・保存しない。
- **DigestDraft 以降**: Digest 生成時に、最新 PolicyDraft の `representativeContextSet` があればプロンプトに追加して「誰向け・何のため・どんな効果か」を踏まえた要約にできる。未設定なら従来どおり。**変数設定（環境変数など）への影響はなし**。既存 API はそのまま返すだけなので、返却 JSON に `representativeContextSet` が増えるだけである。
